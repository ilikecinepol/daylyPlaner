...
Личный финансовый контур включает счета, категории, доходы, расходы, переводы, плановые платежи задач, финансовые показатели существующих целей, бюджеты проектов, аналитику и CSV-отчёт. Балансы выводятся отдельно по валютам и рассчитываются из начального остатка и журнала операций.

REST API доступен под `/api/v1/finance`: `accounts`, `categories`, `transactions`, `planned`, `overview`, `analytics`, `report`; проведение платежа задачи — `POST /api/v1/finance/tasks/{task_id}/execute`. Финансовая привязка общей задачи является личной и не включается в общий serializer задачи.

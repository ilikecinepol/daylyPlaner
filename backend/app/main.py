...
from .models import User,Project,Task,TaskTemplate,KanbanColumn,Reminder,ActivityLog,CalendarConnection,CalendarEventLink,Contact,ProjectRole,ProjectMember,ProjectMemberRole,ChatChannel,ChatMessage
...
def upgrade_legacy_data():
    with next(get_db()) as db:
        ...
        db.flush()
        for t in db.scalars(select(Task).where(Task.start_at==None,Task.date!="")):
            try:t.start_at=datetime.fromisoformat(t.date+"T"+(t.time or "12:00")+":00").replace(tzinfo=timezone.utc);t.all_day=not bool(t.time);t.duration_minutes=t.duration;t.status="completed" if t.completed else "planned"
...
def task_out(db,t):
    ...
    return {"id":t.id,"title":t.title,"description":t.description,"status":t.status,"priority":t.priority,"project_id":t.project_id,"column_id":t.column_id,"start_at":dt(t.start_at),"due_at":dt(t.due_at),"duration_minutes":t.duration_minutes or t.duration,"all_day":t.all_day,"location":t.location,"tags":t.tags or [],"mentions":t.mentions or [],"recurrence_rule":t.recurrence_rule or "","reminder_offsets":[r.offset_minutes for r in reminders],"completed_at":dt(t.completed_at),"sync_version":t.sync_version,"created_at":dt(t.created_at),"updated_at":dt(t.updated_at)}
def project_out(db,p):
    ...
    return {"id":p.id,"name":p.name,"description":p.description,"color":p.color,"team_label":p.team_label or "","owner_id":p.user_id,"columns":[{"id":c.id,"name":c.name,"position":c.position} for c in db.scalars(select(KanbanColumn).where(KanbanColumn.project_id==p.id).order_by(KanbanColumn.position))],"roles":[{"id":r.id,"name":r.name,"color":r.color,"permissions":r.permissions,"position":r.position} for r in roles],"members":members,"channels":[{"id":c.id,"name":c.name,"description":c.description,"position":c.position} for c in db.scalars(select(ChatChannel).where(ChatChannel.project_id==p.id).order_by(ChatChannel.position))]}
def log(db,u,action,task_id=None,changes={}): db.add(ActivityLog(user_id=u.id,task_id=task_id,action=action,changes=changes))
def next_occurrence(value,rule):
    if not value or not rule:return None
    if rule=="DAILY":return value+timedelta(days=1)
    if rule=="WEEKLY":return value+timedelta(days=7)
    if rule=="WEEKDAYS":
        n=value+timedelta(days=1)
        while n.weekday()>4:n+=timedelta(days=1)
        return n
    if rule=="MONTHLY":
        m=value.month%12+1;y=value.year+(value.month//12);return value.replace(year=y,month=m,day=min(value.day,28))
    if rule=="YEARLY":return value.replace(year=value.year+1)
...
def create_task(data:TaskIn,u=Depends(current_user),db:Session=Depends(get_db)):
    ...
    values=data.model_dump(exclude={"reminder_offsets"});t=Task(user_id=u.id,**values,date=data.start_at.date().isoformat() if data.start_at else "",time=data.start_at.strftime("%H:%M") if data.start_at and not data.all_day else "",duration=data.duration_minutes,completed=False)
...
def patch_task(task_id:str,data:TaskPatch,u=Depends(current_user),db:Session=Depends(get_db)):
    ...
    if "status" in patch and patch["status"]=="completed":
        t.completed=True;t.completed_at=datetime.now(timezone.utc)
        ...
        if nxt and not db.scalar(select(Task).where(Task.user_id==u.id,Task.title==t.title,Task.start_at==nxt,Task.deleted_at==None)):
            child=Task(user_id=u.id,project_id=t.project_id,column_id=t.column_id,title=t.title,description=t.description,status="planned",priority=t.priority,start_at=nxt,due_at=(t.due_at+(nxt-t.start_at)) if t.due_at else None,duration_minutes=t.duration_minutes,all_day=t.all_day,location=t.location,tags=t.tags,mentions=t.mentions,recurrence_rule=t.recurrence_rule,date=nxt.date().isoformat(),time="" if t.all_day else nxt.strftime("%H:%M"),duration=t.duration_minutes,column_name=t.column_name,completed=False);db.add(child);db.flush();[db.add(Reminder(task_id=child.id,offset_minutes=r.offset_minutes,channel=r.channel)) for r in db.scalars(select(Reminder).where(Reminder.task_id==t.id))]
    elif "status" in patch:t.completed=False;t.completed_at=None
    if "start_at" in patch:t.date=t.start_at.date().isoformat() if t.start_at else "";t.time=t.start_at.strftime("%H:%M") if t.start_at and not t.all_day else ""
    if "duration_minutes" in patch:t.duration=t.duration_minutes
...
def notifications(u=Depends(current_user),db:Session=Depends(get_db)):
    ...
    for r,t in db.execute(select(Reminder,Task).join(Task,Reminder.task_id==Task.id).where(Task.user_id==u.id,Task.deleted_at==None,Task.completed_at==None,Reminder.sent_at==None,Task.start_at!=None)):
        start=t.start_at.replace(tzinfo=timezone.utc) if t.start_at.tzinfo is None else t.start_at
        if start-timedelta(minutes=r.offset_minutes)<=current<=start:result.append({"id":r.id,"task_id":t.id,"title":t.title,"start_at":dt(t.start_at),"offset_minutes":r.offset_minutes})
    return result
...
def connections(u=Depends(current_user),db:Session=Depends(get_db)):return [{"id":x.id,"provider":x.provider,"account":x.external_account_id} for x in db.scalars(select(CalendarConnection).where(CalendarConnection.user_id==u.id,CalendarConnection.deleted_at==None))]
...
def google_sync(u=Depends(current_user),db:Session=Depends(get_db)):
    ...
    access=google.decrypt(c.access_token_encrypted);exported=imported=0
    for t in db.scalars(select(Task).where(Task.user_id==u.id,Task.deleted_at==None,Task.start_at!=None)):
        link=db.scalar(select(CalendarEventLink).where(CalendarEventLink.task_id==t.id,CalendarEventLink.calendar_connection_id==c.id));start=t.start_at.isoformat();end=(t.start_at+timedelta(minutes=t.duration_minutes)).isoformat();event={"summary":t.title,"description":t.description,"location":t.location,"extendedProperties":{"private":{"plan_task_id":t.id}}}
        if t.all_day:event|={"start":{"date":t.start_at.date().isoformat()},"end":{"date":(t.start_at.date()+timedelta(days=1)).isoformat()}}
        else:event|={"start":{"dateTime":start},"end":{"dateTime":end}}
        remote=google.upsert_event(access,event,link.external_event_id if link else None)
        if not link:link=CalendarEventLink(task_id=t.id,calendar_connection_id=c.id,external_event_id=remote["id"]);db.add(link)
        link.last_synced_at=datetime.now(timezone.utc);link.external_updated_at=datetime.fromisoformat(remote["updated"].replace("Z","+00:00"));exported+=1
    for event in google.list_events(access):
        task_id=event.get("extendedProperties",{}).get("private",{}).get("plan_task_id");t=db.get(Task,task_id) if task_id else None
        if not t or t.user_id!=u.id:continue
        updated=datetime.fromisoformat(event["updated"].replace("Z","+00:00"));local=t.updated_at.replace(tzinfo=timezone.utc) if t.updated_at and t.updated_at.tzinfo is None else t.updated_at
        if local and updated>local:
            start=event.get("start",{});value=start.get("dateTime") or start.get("date");t.start_at=datetime.fromisoformat(value.replace("Z","+00:00"));t.all_day="date" in start;t.title=event.get("summary",t.title);t.location=event.get("location",t.location);t.sync_version+=1;imported+=1
    db.commit();return {"exported":exported,"imported":imported,"synced_at":datetime.now(timezone.utc).isoformat()}
...
def google_callback(code:str,state:str,db:Session=Depends(get_db)):
    ...
    c.access_token_encrypted=google.encrypt(data.get("access_token",""));c.refresh_token_encrypted=google.encrypt(data.get("refresh_token",""));db.add(c);db.commit();return RedirectResponse("/?calendar=connected")
